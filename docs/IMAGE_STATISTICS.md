...
- 使用“定位像素”输入绝对坐标后，该坐标成为持久的后备联动点；鼠标进入画布时临时接管，离开画布后恢复最近一次手动定位点。切换文件、描述符或 frame 会清除该状态。
...
停靠视图直接共享联动状态；独立窗口只通过 `statistics:link` 与 `statistics:hover` 交换快照身份和坐标，不在 pointer move 上重复传输完整 `AnalysisResult`。联动点携带 `generation / analysisRevision / frame`，独立窗口必须针对当前统计快照校验后才显示，避免旧结果标到新帧或新 ROI 上。
