...
- 屏幕宽度不足时可以为绘制聚合相邻 bin；tooltip 必须明确显示聚合的 DN 范围及范围计数，摘要和未来的数据导出仍引用逐 DN 精确结果。
- 绘制层不得通过 `Math.min(...values)`、`Math.max(...values)` 等参数展开处理 16-bit 多通道数组；完整结果域必须使用迭代扫描，避免触发 WebView JavaScript 调用栈上限。
...
屏幕表达保留连续趋势线，便于在总览中观察变化；它不表示两个真实采样坐标之间存在插值数据。横轴局部放大且真实采样点的平均屏幕间距足够时，图表仅在实际存在样本的物理坐标上显示空心圆标记。标记不会生成新数据，Tooltip 也只读取真实 Profile 点。
...
精确 Profile 始终保留在结构化结果中。全范围绘制时，每条曲线最多向图表控件提交 4096 个点，并用 min/max envelope 同时保留每个分桶的局部高低点；横轴缩放到较小范围后重新从精确 Profile 生成数据并恢复该范围内全部原始点。此策略只改变屏幕绘制负载，不改变数值摘要、坐标、统计精度或未来导出语义。

### 像素与 Profile 联动

联动是只读的屏幕定位辅助，不改变源数据、统计结果或任何视图范围：

- 主画布达到 `12×`（每个源像素至少 12 个屏幕像素）后，鼠标所在像素同时启用整行与整列的低透明度柔光边框；低于阈值时不显示联动，避免缩小时形成大面积遮挡。
- 使用“定位像素”输入绝对坐标后会立即高亮该位置；画布或 Profile 的下一次鼠标交互会接管并清除这次手动定位高亮，鼠标离开后不会恢复旧定位点。切换文件、描述符或 frame 同样会清除联动状态。
- 当前统计 ROI 包含联动像素时，Row Profile 在源 `y`、Column Profile 在源 `x` 处同时显示引导线，并为所有当前可见的有效分组显示精确 Profile 点；目标在 ROI 外时不伪造统计点。
- Profile 的联动点直接从结构化结果中的精确数组按源坐标查找，不从最多 4096 点的屏幕曲线反推，因此显示降采样不会改变标记数值。
- 鼠标位于 Row Profile 时只在主画布高亮对应物理行，位于 Column Profile 时只高亮对应物理列；Histogram 的 DN 桶不能唯一映射到空间行列，因此不参与联动。
- 主画布和三个图表各自保留既有缩放、平移和手动坐标范围。目标落在当前可见范围外时只保留联动状态，不自动滚动、居中或改变轴范围；重新把目标移入可见范围后自然出现。
- 联动高亮是临时 UI 叠加，不进入当前画面抓拍、完整预览或 RAW 导出。

停靠视图直接共享联动状态；独立窗口只通过 `statistics:link` 与 `statistics:hover` 交换快照身份和坐标，不在 pointer move 上重复传输完整 `AnalysisResult`。画布与 Profile 采用“最后活动输入源”仲裁，因此即使跨原生窗口时未及时收到 `pointerleave` 或 `globalout`，新悬停也会接管旧状态。联动点携带 `generation / analysisRevision / frame`，独立窗口必须针对当前统计快照校验后才显示，避免旧结果标到新帧或新 ROI 上。
...
- Rust 扫描热循环使用连续的语义组、行列 Profile 和原子平面累加器；固定索引替代逐像素字符串匹配与有序映射查找。
- IPC 只返回摘要、精确 Histogram、Profile 和溯源元数据。
- 图表域扫描不得展开长数组；Histogram 和 Profile 只在显示层生成有界数据，三个图表分帧初始化并分别报告错误。
...
- 第一次进入或没有已保存偏好时以主窗口侧方停靠形式打开，保持功能启用后的高内聚形态并适配宽屏；用户显式切换底部/侧方停靠或摘出后记住该呈现偏好。
- 停靠视图提供“摘出”入口，独立窗口提供“停靠到主窗口”入口；切换呈现方式不重新计算。
- 独立窗口使用 Windows 原生标题栏和关闭按钮，标题按“eRAW - 模块名”命名（中文为“eRAW - 图像统计”）；窗口内容不重复绘制标题栏或关闭图标，停靠视图保留文字形式的关闭入口。
- 统计视图已打开时，画布右键入口展开停靠区域或聚焦独立窗口。
- 停靠视图可在底部与参数栏对侧之间切换；底部可调整高度，侧方可调整宽度。侧方最大宽度约为参数栏之外可用工作区的三分之二，并至少为画布保留 320 px；窗口宽度不足时回退到底部，不自动收起参数栏。
...
“图像格式”板块的参数成功提交后自动执行相同的视图重置，同时以新文档 generation 重新统计；统计页面、ROI、停靠方式和外层尺寸保持不变。描述符提交采用事务语义：后端接受后才保存参数并废弃旧结果，拒绝时恢复最后一个有效描述符和统计结果，不以关闭统计页面掩盖参数错误。
...
- 每个图表同时绘制全部可用语义通道，并分别提供曲线显隐开关；开关只更新目标图表，不重建另外两张图。All 与 G 通过线宽和虚线与其他曲线区分，颜色明暗从当前主题变量派生。隐藏曲线不参与 Tooltip；悬停只更新十字指示器、采样点和 Tooltip，normal、emphasis 与 blur 状态使用完全相同的曲线样式，不代替用户强调或淡化通道。
- Row/Column Profile 在总览中保留连续趋势线；局部放大到真实采样点具备足够屏幕间距时，以主题表面色填充、通道色描边的空心圆标出实际物理坐标。连续采样仍可保持纯线条，稀疏 CFA 子通道则可辨认坐标间隔。
- Tooltip 和十字指示器用于读取坐标与数值；普通滚轮用于滚动纵向统计页面，`Ctrl + 滚轮`缩放横轴，`Shift + 滚轮`缩放纵轴，`Ctrl + Shift + 滚轮`同步缩放双轴。图内拖动和底部范围滑块继续用于缩放及平移横轴范围。
...
- 主窗口全屏切换或普通尺寸变化只调整图表画布尺寸；横轴范围没有实际变化时，不重新采样或更新 Row/Column Profile 曲线。
...
- V0.4.1 将首次呈现改为侧方停靠并按可用工作区动态放宽宽度；增加 Shift/组合滚轮纵向与双轴缩放，修正悬停淡出和跨图重绘，并在图像格式成功提交后以事务方式重置和刷新统计视图。
- V0.4.2 禁用图表曲线的悬停强调，并锁定 normal、emphasis 与 blur 状态的颜色、线型、线宽和透明度；Tooltip、十字指示器与采样点读取保持不变。
- V0.4.5 保留 Profile 连续趋势线，并在局部放大后仅于真实采样坐标显示自适应空心标记，避免稀疏 CFA 曲线暗示不存在的中间数据。
