...
- 统计视图不分页，固定显示包含统计摘要、DN 直方图、行剖面和列剖面的单一总览。
...
- 停靠与独立形态使用一致的单页总览；独立窗口只通过响应式 dashboard 扩展信息密度，不增加页面层级。
...
- 回到默认总览状态，折叠 QCFA 子相位等高级层级。
- 摘要显示核心字段，详细字段恢复默认折叠状态。
- 直方图使用普通计数、线性纵轴、自动坐标范围和默认缩放，并显示全部 CFA 采样点与 R/Gr/Gb/B 子通道比较。
- 行列剖面使用均值、自动坐标范围和默认缩放，只显示全部 CFA 采样点。
- 内部图表和分区尺寸恢复默认值。
...
- 统计窗口只提供一个“保存 PNG 报告”按钮，不提供分页、报告预览、单图复制或单图保存入口。
- 用户确认保存路径后，直接从当前结构化统计快照生成完整报告。
...
- V0.3.3 将统计视图收敛为单页总览，报告通过唯一按钮直接保存为 PNG；统计标签和报告文字随七种界面语言本地化。
- ROI 选区使用固定两像素高对比虚线和轻量明暗阴影，避免在亮部、暗部或复杂纹理上丢失边界。
