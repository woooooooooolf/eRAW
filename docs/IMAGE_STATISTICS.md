...
ROI 是主窗口级状态，复用 `SelectionModel` 和 SVG 叠加层，不由统计窗口创建或销毁。内部矩形继续使用原图坐标和半开区间：
...
- 统计视图不分页，按 ROI 信息、数据完整性、采样点类型、DN 直方图与摘要、行剖面、列剖面的顺序纵向排列；各板块以主题对比色分隔线区分。
...
- 统计窗口选择通道或操作图表不改变主窗口 RAW/CFA/Remosaic/Demosaic 预览模式。
...
主工具栏在“适应窗口”左侧提供独立的“区域选择”组合控件，并用分隔线与右侧视图按钮区分：
...
- 没有 ROI 时默认统计完整当前帧。
- 鼠标方式把左键按下位置作为起点、松开位置作为包含式终点；完成后自动恢复平移模式。
...
- Tooltip 和十字指示器用于读取坐标与数值；图内拖动、滚轮和底部范围滑块用于缩放及平移横轴范围。
