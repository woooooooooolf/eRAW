...
- 统计视图只有一个实例，可以停靠在主窗口底部，也可以“摘出”为独立窗口；两种形态不同时维护两份结果。
- 第一次进入或没有已保存偏好时以主窗口底部停靠形式打开，保持功能启用后的高内聚形态；用户显式停靠或摘出后记住该呈现偏好。
- 停靠视图提供“摘出”入口，独立窗口提供“停靠到主窗口”入口；切换呈现方式不重新计算。
- 统计视图已打开时，画布右键入口展开停靠区域或聚焦独立窗口。
- 底部停靠区域可折叠、可调整高度，不与可放在左右两侧的参数栏冲突。
- 统计视图包含“摘要 / Histogram / Row Profile / Column Profile / 报告”。
- 没有 ROI 时显示整帧；矩形 ROI 完成后自动重新计算。
- 关闭统计视图只隐藏结果和 ROI 叠加边框，不修改 ROI 数据、RAW 参数或当前预览；再次打开时恢复选区边框和结果。
- 计算中在统计视图内部显示进度和取消入口，不使用阻塞提示框。
- 当前实现阶段只维护一份“当前文件、当前帧、当前 ROI”结果，不引入 ImageJ 式多测量历史或跨文件结果表。
- 统计窗口选择通道、图表或报告样式不改变主窗口 RAW/CFA/Remosaic/Demosaic 预览模式。
- 独立窗口跟随主窗口的当前文件、描述符、frame 和 ROI；关闭文件后保持窗口但进入空状态。
- 独立窗口中的“选择 ROI”会激活主窗口并进入矩形选择模式。
- 停靠与独立形态使用一致的信息架构和 `总览 | Histogram | Profiles | 报告` 导航；停靠形态突出当前 Tab，独立窗口使用响应式 dashboard 充分利用较大的屏幕空间。
...
- 没有 ROI 时默认统计完整当前帧。
- “选择区域”激活主窗口和矩形选择模式，鼠标释放后自动切换到“选区”并开始统计。
- 已有 ROI 时可以在“整帧/选区”之间切换；切换到整帧不删除 ROI。
- “清除选区”才删除 ROI 并恢复整帧统计。
- `Esc` 只取消正在进行的框选，不删除此前已经完成的 ROI。
- 同尺寸 frame 切换继续保留 ROI；图像尺寸变化时清除。
- 统计视图关闭时隐藏 ROI 边框但保留其数据；摘出为独立窗口时统计功能仍然打开，因此边框继续显示。
- 右键菜单入口不增加“统计此处”等位置相关语义；整帧或选区范围由统计视图中的明确控件决定。

### 一键重置

统计视图标题栏在停靠和独立两种形态下都提供“重置视图”操作。该操作无数据破坏性，直接执行而不弹确认框。

重置后的默认展示状态：

- 返回 All CFA；MONO 返回 Y。
- 回到总览区域，折叠 QCFA 子相位等高级层级。
- 摘要显示核心字段，详细字段恢复默认折叠状态。
- Histogram 恢复普通 Histogram、线性纵轴、自动坐标范围和默认缩放，并显示 All CFA 与 R/Gr/Gb/B 子通道比较。
- Profile 恢复 Mean、自动坐标范围和默认缩放，只显示 All CFA。
- 报告预览恢复中性浅色、高分辨率 16:9 默认版式。
- 内部图表和分区尺寸恢复默认值。
...
All CFA 默认报告同时包含总体摘要和子通道比较：Histogram 显示 All CFA 与 R/Gr/Gb/B，摘要对照区列出各语义通道的 Mean、Standard Deviation、Zero DN 和 Full-scale DN；Row/Column Profile 默认保持 All CFA，避免报告首屏被稀疏曲线淹没。
...
- `src/statistics-panel.ts` 使用 Canvas 2D 绘制面板图表和独立于应用主题的 16:9 浅色报告，不截取软件窗口。
