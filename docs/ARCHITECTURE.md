...
底部停靠、参数栏对侧的侧方停靠和独立统计窗口共享唯一 Analysis State 与结构化结果，同一时间只有一个统计视图；切换载体不复制任务或重新扫描 RAW。统计视图默认隐藏，通过已打开图像的画布右键菜单进入，首次以底部停靠形式打开；独立窗口使用系统原生标题栏，内部不重复绘制标题和关闭按钮。曲线显隐、横纵轴范围及三种布局各自的图表高度由 `statistics-view-state.ts` 独立维护并持久化，不改变主窗口预览或 ROI。面板重建内容时保留自身滚动位置，短暂的无结果加载态不能覆盖已保存位置。
