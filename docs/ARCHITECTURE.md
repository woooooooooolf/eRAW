...
| `src/help-content.ts` / `src/help-content-localized.ts` / `src/help-math.ts` / `src/help-window.ts` | 按帮助路由惰性加载的分组、分篇七语技术参考内容，共享 LaTeX 公式源到原生 MathML 的渲染、单篇路由、前后篇导航，以及与主窗口语言、主题即时同步并原位重建内容的独立帮助窗口 |
