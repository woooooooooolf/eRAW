...
前端保留 IPC 返回的精确 Histogram 与 Profile，但不会把超大数组直接全部交给 ECharts。Histogram 绘制聚合为有明确 DN 范围的有限显示桶；Row/Column Profile 在全范围使用最多 4096 点的 min/max envelope 保留局部尖峰，缩放到较小范围后从精确结果恢复范围内全部原始点。数据域扫描使用迭代算法，不以数组展开作为函数参数；三个图表分帧初始化并独立捕获错误，因此单张图异常不会留下无说明的空白面板，也不会阻断其他图表。
