...
const CATALOG = {
  ...
  "roi.mouseShortcut": message("Mouse ROI (R)", "鼠标框选 ROI (R)", "滑鼠框選 ROI (R)", "マウス ROI (R)", "ROI con ratón (R)", "ROI à la souris (R)", "Maus-ROI (R)"),
  ...
  "roi.coordinatesShortcut": message("Coordinate ROI (Shift+R)", "坐标 ROI (Shift+R)", "座標 ROI (Shift+R)", "座標 ROI (Shift+R)", "ROI por coordenadas (Shift+R)", "ROI par coordonnées (Shift+R)", "Koordinaten-ROI (Shift+R)"),
  ...
  "roi.dialogEyebrow": message("REGION OF INTEREST", "感兴趣区域", "感興趣區域", "関心領域", "REGIÓN DE INTERÉS", "RÉGION D’INTÉRÊT", "REGION VON INTERESSE"),
  "roi.dialogTitle": message("Enter ROI coordinates", "输入 ROI 坐标", "輸入 ROI 座標", "ROI 座標を入力", "Introducir coordenadas de ROI", "Saisir les coordonnées du ROI", "ROI-Koordinaten eingeben"),
  "roi.dialogHint": message("Coordinates start at (0, 0) in the top-left corner. Both endpoints are included.", "坐标从图像左上角的 (0, 0) 开始，起止端点均包含在 ROI 中。", "座標從影像左上角的 (0, 0) 開始，起迄端點皆包含在 ROI 中。", "座標は画像左上の (0, 0) から始まり、開始点と終了点の両方を ROI に含みます。", "Las coordenadas comienzan en (0, 0) en la esquina superior izquierda e incluyen ambos extremos.", "Les coordonnées commencent à (0, 0) dans le coin supérieur gauche et incluent les deux extrémités.", "Die Koordinaten beginnen oben links bei (0, 0); beide Endpunkte gehören zum ROI."),
  "roi.xStart": message("X start", "X 起始", "X 起始", "X 開始", "Inicio X", "Début X", "X-Start"),
  "roi.xEnd": message("X end", "X 截止", "X 截止", "X 終了", "Fin X", "Fin X", "X-Ende"),
  "roi.yStart": message("Y start", "Y 起始", "Y 起始", "Y 開始", "Inicio Y", "Début Y", "Y-Start"),
  "roi.yEnd": message("Y end", "Y 截止", "Y 截止", "Y 終了", "Fin Y", "Fin Y", "Y-Ende"),
  "roi.apply": message("Apply ROI", "应用 ROI", "套用 ROI", "ROI を適用", "Aplicar ROI", "Appliquer le ROI", "ROI anwenden"),
  "roi.availableRange": message("Available: X[0, {maxX}] Y[0, {maxY}]", "可用范围：X[0, {maxX}] Y[0, {maxY}]", "可用範圍：X[0, {maxX}] Y[0, {maxY}]", "有効範囲：X[0, {maxX}] Y[0, {maxY}]", "Disponible: X[0, {maxX}] Y[0, {maxY}]", "Disponible : X[0, {maxX}] Y[0, {maxY}]", "Verfügbar: X[0, {maxX}] Y[0, {maxY}]"),
  "roi.error.integer": message("Enter an integer for every coordinate.", "每个坐标都必须输入整数。", "每個座標都必須輸入整數。", "すべての座標に整数を入力してください。", "Introduzca un número entero para cada coordenada.", "Saisissez un entier pour chaque coordonnée.", "Geben Sie für jede Koordinate eine Ganzzahl ein."),
  "roi.error.xOrder": message("X end must be greater than or equal to X start.", "X 截止坐标必须大于或等于 X 起始坐标。", "X 截止座標必須大於或等於 X 起始座標。", "X 終了は X 開始以上である必要があります。", "El fin X debe ser mayor o igual que el inicio X.", "La fin X doit être supérieure ou égale au début X.", "X-Ende muss größer oder gleich X-Start sein."),
  "roi.error.yOrder": message("Y end must be greater than or equal to Y start.", "Y 截止坐标必须大于或等于 Y 起始坐标。", "Y 截止座標必須大於或等於 Y 起始座標。", "Y 終了は Y 開始以上である必要があります。", "El fin Y debe ser mayor o igual que el inicio Y.", "La fin Y doit être supérieure ou égale au début Y.", "Y-Ende muss größer oder gleich Y-Start sein."),
  "roi.error.bounds": message("Coordinates must stay within X[0, {maxX}] Y[0, {maxY}].", "坐标必须位于 X[0, {maxX}] Y[0, {maxY}] 范围内。", "座標必須位於 X[0, {maxX}] Y[0, {maxY}] 範圍內。", "座標は X[0, {maxX}] Y[0, {maxY}] の範囲内で指定してください。", "Las coordenadas deben estar dentro de X[0, {maxX}] Y[0, {maxY}].", "Les coordonnées doivent rester dans X[0, {maxX}] Y[0, {maxY}].", "Die Koordinaten müssen innerhalb X[0, {maxX}] Y[0, {maxY}] liegen."),
  "roi.mouseUnavailable": message("Mouse ROI selection is available in RAW intensity and CFA views.", "鼠标 ROI 框选仅在 RAW 强度和 CFA 视图中可用。", "滑鼠 ROI 框選僅在 RAW 強度和 CFA 檢視中可用。", "マウスによる ROI 選択は RAW 強度および CFA 表示で利用できます。", "La selección de ROI con el ratón está disponible en las vistas RAW y CFA.", "La sélection du ROI à la souris est disponible dans les vues RAW et CFA.", "Die ROI-Auswahl mit der Maus ist in der RAW- und CFA-Ansicht verfügbar."),

  "statistics.title": message("Image Statistics", "图像统计", "影像統計", "画像統計", "Estadísticas de imagen", "Statistiques d’image", "Bildstatistik"),
  "statistics.noFile": message("No file", "未打开文件", "未開啟檔案", "ファイルなし", "Sin archivo", "Aucun fichier", "Keine Datei"),
  ...
  "statistics.dock": message("Dock to main window", "停靠到主窗口", "停靠至主視窗", "メインウィンドウに戻す", "Acoplar a la ventana principal", "Ancrer à la fenêtre principale", "Im Hauptfenster andocken"),
  "statistics.dockSide": message("Dock at side", "停靠到侧方", "停靠至側方", "側面にドッキング", "Acoplar al lateral", "Ancrer sur le côté", "Seitlich andocken"),
  "statistics.dockBottom": message("Dock at bottom", "停靠到底部", "停靠至底部", "下部にドッキング", "Acoplar abajo", "Ancrer en bas", "Unten andocken"),
  "statistics.sideUnavailable": message("The window is too narrow for side docking", "当前窗口宽度不足以侧方停靠", "目前視窗寬度不足以側方停靠", "側面にドッキングするにはウィンドウ幅が不足しています", "La ventana es demasiado estrecha para acoplar al lateral", "La fenêtre est trop étroite pour un ancrage latéral", "Das Fenster ist für seitliches Andocken zu schmal"),
  "statistics.range": message("Range", "范围", "範圍", "範囲", "Rango", "Plage", "Bereich"),
  "statistics.fullFrame": message("Full frame", "整帧", "整個影格", "フレーム全体", "Fotograma completo", "Image entière", "Vollbild"),
  "statistics.selection": message("Selection", "选区", "選取範圍", "選択範囲", "Selección", "Sélection", "Auswahl"),
  "statistics.selectRegion": message("Select region", "选择区域", "選擇區域", "領域を選択", "Seleccionar región", "Sélectionner une région", "Bereich auswählen"),
  "statistics.clearSelection": message("Clear selection", "清除选区", "清除選取範圍", "選択を消去", "Borrar selección", "Effacer la sélection", "Auswahl löschen"),
  "statistics.roiOrigin": message("Coordinates start at (0, 0) in the top-left corner of the source image; both endpoints are included.", "坐标以源图像左上角为 (0, 0) 起点，起止端点均包含在 ROI 中。", "座標以來源影像左上角為 (0, 0) 起點，起迄端點皆包含在 ROI 中。", "元画像の左上を (0, 0) とし、開始点と終了点の両方を ROI に含みます。", "Las coordenadas comienzan en (0, 0) en la esquina superior izquierda de la imagen de origen e incluyen ambos extremos.", "Les coordonnées commencent à (0, 0) dans le coin supérieur gauche de l’image source et incluent les deux extrémités.", "Die Koordinaten beginnen oben links im Quellbild bei (0, 0); beide Endpunkte gehören zum ROI."),
  "statistics.integrityComplete": message("All {count} samples in the ROI are valid.", "ROI 内 {count} 个采样点均有效。", "ROI 內 {count} 個取樣點均有效。", "ROI 内の {count} サンプルはすべて有効です。", "Las {count} muestras del ROI son válidas.", "Les {count} échantillons du ROI sont tous valides.", "Alle {count} Stichproben im ROI sind gültig."),
  "statistics.integrityMissing": message("Expected {expected}, valid {valid}, missing {missing}; missing samples are excluded from statistics.", "预期 {expected}，有效 {valid}，缺失 {missing}；缺失采样不参与统计。", "預期 {expected}，有效 {valid}，缺失 {missing}；缺失取樣不參與統計。", "期待 {expected}、有効 {valid}、欠損 {missing}。欠損サンプルは統計から除外されます。", "Esperadas {expected}, válidas {valid}, ausentes {missing}; las muestras ausentes se excluyen de las estadísticas.", "Attendus {expected}, valides {valid}, manquants {missing} ; les échantillons manquants sont exclus des statistiques.", "Erwartet {expected}, gültig {valid}, fehlend {missing}; fehlende Stichproben werden von der Statistik ausgeschlossen."),
  "statistics.samplingSites": message("CFA sampling sites", "CFA 采样点类型", "CFA 取樣點類型", "CFA サンプリングサイト", "Sitios de muestreo CFA", "Sites d’échantillonnage CFA", "CFA-Abtaststellen"),
  "statistics.chartInteraction": message("Hover to inspect; Ctrl+wheel zooms X, Shift+wheel zooms Y, and Ctrl+Shift+wheel zooms both.", "悬停查看数值；Ctrl+滚轮缩放横轴，Shift+滚轮缩放纵轴，Ctrl+Shift+滚轮同时缩放。", "懸停檢視數值；Ctrl+滾輪縮放橫軸，Shift+滾輪縮放縱軸，Ctrl+Shift+滾輪同時縮放。", "ホバーで値を確認できます。Ctrl+ホイールで横軸、Shift+ホイールで縦軸、Ctrl+Shift+ホイールで両軸を拡大縮小します。", "Pase el cursor para inspeccionar; Ctrl+rueda amplía X, Shift+rueda amplía Y y Ctrl+Shift+rueda amplía ambos ejes.", "Survolez pour inspecter ; Ctrl+molette zoome sur X, Maj+molette sur Y et Ctrl+Maj+molette sur les deux axes.", "Werte beim Überfahren prüfen; Strg+Mausrad zoomt X, Umschalt+Mausrad Y und Strg+Umschalt+Mausrad beide Achsen."),
  "statistics.profileSamplingHint": message("Zoom in to reveal actual sample coordinates.", "放大后显示实际采样坐标。", "放大後顯示實際取樣座標。", "拡大すると実際のサンプル座標を表示します。", "Amplíe para mostrar las coordenadas de muestreo reales.", "Zoomez pour afficher les coordonnées d’échantillonnage réelles.", "Beim Vergrößern werden die tatsächlichen Abtastkoordinaten angezeigt."),
  "statistics.curves": message("Visible curves", "显示曲线", "顯示曲線", "表示曲線", "Curvas visibles", "Courbes visibles", "Sichtbare Kurven"),
  "statistics.resetYAxis": message("Reset Y-axis range", "重置 Y 轴范围", "重設 Y 軸範圍", "Y 軸範囲をリセット", "Restablecer rango del eje Y", "Réinitialiser la plage de l’axe Y", "Y-Achsenbereich zurücksetzen"),
  "statistics.visibleRange": message("Visible horizontal range", "横轴可见范围", "橫軸可見範圍", "表示する水平範囲", "Rango horizontal visible", "Plage horizontale visible", "Sichtbarer horizontaler Bereich"),
  ...
  "statistics.allCfa": message("All CFA sites", "全部 CFA 采样点", "全部 CFA 取樣點", "全 CFA サイト", "Todos los sitios CFA", "Tous les sites CFA", "Alle CFA-Sites"),
  "statistics.count": message("Count", "计数", "計數", "度数", "Recuento", "Effectif", "Anzahl"),
  "statistics.logCount": message("log10(Count + 1)", "log10(计数 + 1)", "log10(計數 + 1)", "log10(度数 + 1)", "log10(recuento + 1)", "log10(effectif + 1)", "log10(Anzahl + 1)"),
  "statistics.histogram": message("DN Histogram", "DN 直方图", "DN 直方圖", "DN ヒストグラム", "Histograma de DN", "Histogramme DN", "DN-Histogramm"),
  "statistics.histogramCumulative": message("Cumulative DN Histogram", "累计 DN 直方图", "累計 DN 直方圖", "累積 DN ヒストグラム", "Histograma acumulado de DN", "Histogramme DN cumulé", "Kumulatives DN-Histogramm"),
  "statistics.cumulative": message("Cumulative", "累计", "累計", "累積", "Acumulado", "Cumulé", "Kumulativ"),
  "statistics.logScale": message("Log scale", "对数纵轴", "對數縱軸", "対数軸", "Escala logarítmica", "Échelle logarithmique", "Logarithmische Skala"),
  "statistics.mean": message("Mean", "均值", "平均值", "平均", "Media", "Moyenne", "Mittelwert"),
  "statistics.standardDeviation": message("Standard deviation", "标准差", "標準差", "標準偏差", "Desviación estándar", "Écart-type", "Standardabweichung"),
  "statistics.standardDeviationShort": message("Std. dev.", "标准差", "標準差", "標準偏差", "Desv. est.", "Écart-type", "Std.-Abw."),
  "statistics.meanDn": message("Mean DN", "DN 均值", "DN 平均值", "平均 DN", "DN medio", "DN moyen", "Mittlerer DN-Wert"),
  "statistics.stdDevDn": message("DN standard deviation", "DN 标准差", "DN 標準差", "DN 標準偏差", "Desviación estándar de DN", "Écart-type DN", "DN-Standardabweichung"),
  "statistics.sourceCoordinate": message("Source coordinate", "原图坐标", "原圖座標", "元画像座標", "Coordenada de origen", "Coordonnée source", "Quellkoordinate"),
  "statistics.rowProfile": message("Row Profile", "行剖面", "列剖面", "行プロファイル", "Perfil de filas", "Profil des lignes", "Zeilenprofil"),
  "statistics.columnProfile": message("Column Profile", "列剖面", "行剖面", "列プロファイル", "Perfil de columnas", "Profil des colonnes", "Spaltenprofil"),
  "statistics.minimumMaximumDn": message("Minimum / Maximum DN", "最小值 / 最大值 DN", "最小值 / 最大值 DN", "最小 / 最大 DN", "DN mínimo / máximo", "DN minimum / maximum", "DN-Minimum / -Maximum"),
  "statistics.meanMedianDn": message("Mean / Median DN", "均值 / 中位数 DN", "平均值 / 中位數 DN", "平均 / 中央値 DN", "Media / mediana de DN", "Moyenne / médiane DN", "DN-Mittelwert / -Median"),
  "statistics.modeDn": message("Mode DN", "众数 DN", "眾數 DN", "最頻 DN", "Moda de DN", "Mode DN", "Modalwert DN"),
  "statistics.varianceStdDev": message("Variance / Std. deviation", "方差 / 标准差", "變異數 / 標準差", "分散 / 標準偏差", "Varianza / desv. estándar", "Variance / écart-type", "Varianz / Standardabw."),
  "statistics.percentiles": message("P1 / P5 / P95 / P99", "P1 / P5 / P95 / P99 分位数", "P1 / P5 / P95 / P99 百分位數", "P1 / P5 / P95 / P99 パーセンタイル", "Percentiles P1 / P5 / P95 / P99", "Percentiles P1 / P5 / P95 / P99", "Perzentile P1 / P5 / P95 / P99"),
  "statistics.zeroFullScaleDn": message("Zero / Full-scale DN", "零值 / 满量程 DN", "零值 / 滿量程 DN", "ゼロ / フルスケール DN", "DN cero / escala completa", "DN zéro / pleine échelle", "Null- / Vollskalen-DN"),
  "statistics.zeroFullScaleShort": message("Zero / Full scale", "零值 / 满量程", "零值 / 滿量程", "ゼロ / フルスケール", "Cero / escala comp.", "Zéro / pleine éch.", "Null / Vollskala"),
  "statistics.frame": message("Frame", "帧", "影格", "フレーム", "Fotograma", "Image", "Frame"),
  "statistics.bit": message("bit", "位", "位元", "ビット", "bits", "bits", "Bit"),
  "statistics.phase": message("phase", "相位", "相位", "位相", "fase", "phase", "Phase"),
  "statistics.saveReport": message("Save PNG report…", "保存 PNG 报告…", "儲存 PNG 報告…", "PNG レポートを保存…", "Guardar informe PNG…", "Enregistrer le rapport PNG…", "PNG-Bericht speichern…"),
  "statistics.reportSaved": message("PNG report saved", "PNG 报告已保存", "PNG 報告已儲存", "PNG レポートを保存しました", "Informe PNG guardado", "Rapport PNG enregistré", "PNG-Bericht gespeichert"),
  "statistics.reportTitle": message("eRAW Image Statistics Report", "eRAW 图像统计报告", "eRAW 影像統計報告", "eRAW 画像統計レポート", "Informe de estadísticas de imagen eRAW", "Rapport de statistiques d’image eRAW", "eRAW-Bildstatistikbericht"),
  "statistics.channelComparison": message("CFA channel comparison", "CFA 通道对照", "CFA 通道對照", "CFA チャンネル比較", "Comparación de canales CFA", "Comparaison des canaux CFA", "CFA-Kanalvergleich"),
  "statistics.disclaimer": message("Single-frame descriptive L0 CFA DN statistics; this report is not an EMVA 1288 compliant measurement.", "单帧 L0 CFA DN 描述性统计；本报告不构成 EMVA 1288 合规测量。", "單影格 L0 CFA DN 描述性統計；本報告不構成 EMVA 1288 合規測量。", "単一フレームの L0 CFA DN 記述統計です。本レポートは EMVA 1288 準拠測定ではありません。", "Estadística descriptiva DN CFA L0 de un solo fotograma; este informe no es una medición conforme a EMVA 1288.", "Statistiques descriptives DN CFA L0 sur une seule image ; ce rapport ne constitue pas une mesure conforme EMVA 1288.", "Deskriptive L0-CFA-DN-Statistik eines Einzelbilds; dieser Bericht ist keine EMVA-1288-konforme Messung."),
  "backend.analysisTaskFailed": message("Image statistics calculation failed", "图像统计计算失败", "影像統計計算失敗", "画像統計の計算に失敗しました", "Error al calcular las estadísticas de imagen", "Échec du calcul des statistiques d’image", "Berechnung der Bildstatistik fehlgeschlagen"),
  "backend.analysisInvalidRoi": message("The statistics ROI is outside the valid image range", "统计 ROI 超出有效图像范围", "統計 ROI 超出有效影像範圍", "統計 ROI が有効な画像範囲外です", "El ROI estadístico está fuera del rango válido", "Le ROI statistique est hors de la plage valide", "Der Statistik-ROI liegt außerhalb des gültigen Bildbereichs"),
  "backend.analysisInvalidFrame": message("The statistics frame is unavailable", "统计帧不可用", "統計影格不可用", "統計対象のフレームは利用できません", "El fotograma estadístico no está disponible", "L’image statistique n’est pas disponible", "Der Statistik-Frame ist nicht verfügbar"),
  "backend.displayRangeTaskFailed": message("RAW display normalization failed", "RAW 显示归一化失败", "RAW 顯示正規化失敗", "RAW 表示の正規化に失敗しました", "Error al normalizar la visualización RAW", "Échec de la normalisation de l’affichage RAW", "RAW-Anzeigenormalisierung fehlgeschlagen"),
  "backend.displayRangeInvalidFrame": message("The RAW display-normalization frame is unavailable", "RAW 显示归一化帧不可用", "RAW 顯示正規化影格不可用", "RAW 表示正規化の対象フレームは利用できません", "El fotograma para normalizar la visualización RAW no está disponible", "L’image de normalisation de l’affichage RAW n’est pas disponible", "Der Frame für die RAW-Anzeigenormalisierung ist nicht verfügbar"),
  "backend.displayInvalidWindow": message("The RAW display black and white points are invalid", "RAW 显示黑点与白点无效", "RAW 顯示黑點與白點無效", "RAW 表示の黒点と白点が無効です", "Los puntos negro y blanco de la visualización RAW no son válidos", "Les points noir et blanc de l’affichage RAW ne sont pas valides", "Schwarz- und Weißpunkt der RAW-Anzeige sind ungültig"),
  ...
  "toolbar.helpHint": message("Open the user manual", "打开软件使用手册", "開啟軟體使用手冊", "ユーザーマニュアルを開く", "Abrir el manual de usuario", "Ouvrir le manuel d’utilisation", "Benutzerhandbuch öffnen"),
  ...
  "presentation.rawExposure": message("RAW display exposure", "RAW 显示曝光", "RAW 顯示曝光", "RAW 表示露出", "Exposición de visualización RAW", "Exposition d’affichage RAW", "RAW-Anzeigehelligkeit"),
  "presentation.rawExposureHint": message("Changes only the RAW intensity and CFA mosaic preview and PNG captures; original DN, statistics, and exports remain unchanged", "只改变 RAW 强度与 CFA 点阵的预览和 PNG 抓拍，不改变原始 DN、统计或导出", "僅變更 RAW 強度與 CFA 點陣的預覽及 PNG 擷取，不改變原始 DN、統計或匯出", "RAW 強度と CFA モザイクのプレビューおよび PNG キャプチャーのみを変更し、元の DN、統計、エクスポートは変更しません", "Solo cambia la vista previa de intensidad RAW y mosaico CFA y las capturas PNG; no modifica DN, estadísticas ni exportaciones", "Modifie uniquement l’aperçu d’intensité RAW et de mosaïque CFA ainsi que les captures PNG, sans changer les DN, les statistiques ni les exports", "Ändert nur die Vorschau von RAW-Intensität und CFA-Mosaik sowie PNG-Aufnahmen; ursprüngliche DN, Statistiken und Exporte bleiben unverändert"),
  "presentation.decreaseRawExposure": message("Decrease RAW display exposure", "减小 RAW 显示曝光", "減小 RAW 顯示曝光", "RAW 表示露出を下げる", "Reducir la exposición de visualización RAW", "Réduire l’exposition d’affichage RAW", "RAW-Anzeigehelligkeit verringern"),
  "presentation.increaseRawExposure": message("Increase RAW display exposure", "增加 RAW 显示曝光", "增加 RAW 顯示曝光", "RAW 表示露出を上げる", "Aumentar la exposición de visualización RAW", "Augmenter l’exposition d’affichage RAW", "RAW-Anzeigehelligkeit erhöhen"),
  "presentation.rawWindow": message("RAW black / white point", "RAW 黑点 / 白点", "RAW 黑點 / 白點", "RAW 黒点 / 白点", "Punto negro / blanco RAW", "Point noir / blanc RAW", "RAW-Schwarz- / Weißpunkt"),
  "presentation.rawWindowHint": message("Maps original DN between the black and white points to the full preview range before 8-bit quantization", "在 8-bit 量化之前，将黑点与白点之间的原始 DN 映射到完整预览范围", "在 8-bit 量化之前，將黑點與白點之間的原始 DN 映射到完整預覽範圍", "8-bit 量子化の前に、黒点と白点の間の元 DN をプレビューの全範囲に割り当てます", "Asigna los DN originales entre los puntos negro y blanco a todo el rango de vista previa antes de cuantizar a 8 bits", "Mappe les DN d’origine entre les points noir et blanc sur toute la plage d’aperçu avant la quantification 8 bits", "Bildet ursprüngliche DN zwischen Schwarz- und Weißpunkt vor der 8-Bit-Quantisierung auf den gesamten Vorschaubereich ab"),
  "presentation.rawBlackPoint": message("RAW display black point", "RAW 显示黑点", "RAW 顯示黑點", "RAW 表示黒点", "Punto negro de visualización RAW", "Point noir de l’affichage RAW", "Schwarzpunkt der RAW-Anzeige"),
  "presentation.rawWhitePoint": message("RAW display white point", "RAW 显示白点", "RAW 顯示白點", "RAW 表示白点", "Punto blanco de visualización RAW", "Point blanc de l’affichage RAW", "Weißpunkt der RAW-Anzeige"),
  "presentation.rawAuto": message("RAW auto normalization", "RAW 自动归一化", "RAW 自動正規化", "RAW 自動正規化", "Normalización automática RAW", "Normalisation automatique RAW", "Automatische RAW-Normalisierung"),
  "presentation.rawAutoHint": message("Scans every L0 original DN in the current full frame, ignores missing pixels, and sets the black and white points from the All-channel P1/P99", "精确扫描当前整帧的全部 L0 原始 DN，忽略缺失像素，并使用 All 组 P1/P99 设置黑点与白点", "精確掃描目前整個影格的全部 L0 原始 DN，忽略缺失像素，並使用 All 組 P1/P99 設定黑點與白點", "現在のフレーム全体の L0 元 DN をすべて走査し、欠損画素を除外して All チャンネルの P1/P99 から黒点と白点を設定します", "Examina todos los DN originales L0 del fotograma completo actual, ignora los píxeles ausentes y fija los puntos negro y blanco con P1/P99 del canal All", "Analyse tous les DN L0 d’origine de l’image entière actuelle, ignore les pixels manquants et règle les points noir et blanc selon P1/P99 du canal All", "Durchsucht alle ursprünglichen L0-DN des aktuellen Vollbilds, ignoriert fehlende Pixel und setzt Schwarz- und Weißpunkt aus P1/P99 des All-Kanals"),
  "presentation.autoNormalize": message("P1–P99", "P1–P99", "P1–P99", "P1–P99", "P1–P99", "P1–P99", "P1–P99"),
  "presentation.normalizing": message("Calculating…", "计算中…", "計算中…", "計算中…", "Calculando…", "Calcul…", "Berechnung…"),
  "presentation.resetRaw": message("Reset", "重置", "重設", "リセット", "Restablecer", "Réinitialiser", "Zurücksetzen"),
  "presentation.rawAutoApplied": message("RAW display range set to {blackPoint}–{whitePoint} DN", "RAW 显示范围已设为 {blackPoint}–{whitePoint} DN", "RAW 顯示範圍已設為 {blackPoint}–{whitePoint} DN", "RAW 表示範囲を {blackPoint}–{whitePoint} DN に設定しました", "Rango de visualización RAW establecido en {blackPoint}–{whitePoint} DN", "Plage d’affichage RAW réglée sur {blackPoint}–{whitePoint} DN", "RAW-Anzeigebereich auf {blackPoint}–{whitePoint} DN gesetzt"),
  "presentation.rawAutoUnavailable": message("This frame has no usable DN range; the current RAW display range was kept", "当前帧没有可用的 DN 范围，已保留现有 RAW 显示范围", "目前影格沒有可用的 DN 範圍，已保留現有 RAW 顯示範圍", "このフレームには使用可能な DN 範囲がないため、現在の RAW 表示範囲を維持しました", "Este fotograma no tiene un rango DN utilizable; se conservó el rango de visualización RAW actual", "Cette image ne présente aucune plage DN exploitable ; la plage d’affichage RAW actuelle a été conservée", "Dieser Frame hat keinen nutzbaren DN-Bereich; der aktuelle RAW-Anzeigebereich wurde beibehalten"),
  "presentation.demosaicExposure": message("Demosaic display exposure", "Demosaic 显示曝光", "Demosaic 顯示曝光", "Demosaic 表示露出", "Exposición de visualización Demosaic", "Exposition d’affichage Demosaic", "Demosaic-Anzeigehelligkeit"),
  "presentation.demosaicExposureHint": message("Changes only the full-color Demosaic preview and PNG captures; DN, statistics, and RAW/RGB48 exports remain unchanged", "只改变 Demosaic 全彩图及其 PNG 抓拍，不改变 DN、统计或 RAW/RGB48 导出", "僅變更 Demosaic 全彩影像及其 PNG 擷取，不改變 DN、統計或 RAW/RGB48 匯出", "フルカラー Demosaic プレビューと PNG キャプチャーのみを変更し、DN、統計、RAW/RGB48 エクスポートは変更しません", "Solo cambia la vista Demosaic a todo color y las capturas PNG; no modifica DN, estadísticas ni exportaciones RAW/RGB48", "Modifie uniquement l’aperçu Demosaic en couleurs et les captures PNG, sans changer les DN, les statistiques ni les exports RAW/RGB48", "Ändert nur die vollfarbige Demosaic-Vorschau und PNG-Aufnahmen; DN, Statistiken und RAW/RGB48-Exporte bleiben unverändert"),
  "presentation.decreaseExposure": message("Decrease Demosaic display exposure", "减小 Demosaic 显示曝光", "減小 Demosaic 顯示曝光", "Demosaic 表示露出を下げる", "Reducir la exposición de visualización Demosaic", "Réduire l’exposition d’affichage Demosaic", "Demosaic-Anzeigehelligkeit verringern"),
  "presentation.increaseExposure": message("Increase Demosaic display exposure", "增加 Demosaic 显示曝光", "增加 Demosaic 顯示曝光", "Demosaic 表示露出を上げる", "Aumentar la exposición de visualización Demosaic", "Augmenter l’exposition d’affichage Demosaic", "Demosaic-Anzeigehelligkeit erhöhen"),
  ...
  "presentation.pixelGridColor": message("Pixel grid color", "像素网格颜色", "像素網格色彩", "画素グリッドの色", "Color de la cuadrícula de píxeles", "Couleur de la grille de pixels", "Farbe des Pixelrasters"),
  "presentation.pixelGridColorHint": message("Changes only the high-zoom pixel grid; the image and pixel values remain unchanged", "只改变高倍率像素网格，不改变图像或像素值", "只變更高倍率像素網格，不改變影像或像素值", "高倍率の画素グリッドのみを変更し、画像や画素値は変更しません", "Solo cambia la cuadrícula de píxeles con zoom alto; no modifica la imagen ni los valores", "Modifie uniquement la grille de pixels à fort zoom, sans changer l’image ni les valeurs", "Ändert nur das Pixelraster bei starker Vergrößerung; Bild und Pixelwerte bleiben unverändert"),
  ...
  "shortcuts.mouseRoi": message("Mouse ROI selection", "鼠标框选 ROI", "滑鼠框選 ROI", "マウス ROI 選択", "Selección ROI con ratón", "Sélection ROI à la souris", "ROI mit der Maus auswählen"),
  "shortcuts.coordinateRoi": message("Coordinate ROI selection", "输入坐标 ROI", "輸入座標 ROI", "座標 ROI 選択", "Selección ROI por coordenadas", "Sélection ROI par coordonnées", "ROI-Koordinaten eingeben"),
  "shortcuts.locatePixel": message("Locate pixel", "定位像素", "定位像素", "画素を特定", "Localizar píxel", "Localiser un pixel", "Pixel lokalisieren"),
  "shortcuts.enterZoom": message("Enter zoom ratio", "输入缩放比例", "輸入縮放比例", "ズーム率を入力", "Introducir zoom", "Saisir le zoom", "Zoomfaktor eingeben"),
  "shortcuts.statisticsCapture": message("Statistics and capture", "统计与抓拍", "統計與擷取", "統計とキャプチャ", "Estadísticas y captura", "Statistiques et capture", "Statistik und Aufnahme"),
  "shortcuts.openStatistics": message("Open image statistics", "打开图像统计", "開啟影像統計", "画像統計を開く", "Abrir estadísticas de imagen", "Ouvrir les statistiques d’image", "Bildstatistik öffnen"),
  ...
  "about.charting": message("Interactive statistics charts", "交互式统计图表", "互動式統計圖表", "インタラクティブ統計チャート", "Gráficos estadísticos interactivos", "Graphiques statistiques interactifs", "Interaktive Statistikdiagramme"),
  "about.mathTypesetting": message("LaTeX parsing and MathML equation typesetting", "LaTeX 解析与 MathML 公式排版", "LaTeX 解析與 MathML 公式排版", "LaTeX 解析と MathML 数式組版", "Análisis de LaTeX y composición de fórmulas MathML", "Analyse LaTeX et composition de formules MathML", "LaTeX-Parsing und MathML-Formelsatz"),
  ...
  "capture.menuLabel": message("Image capture", "图像抓拍", "影像擷取", "画像キャプチャ", "Captura de imagen", "Capture d’image", "Bildaufnahme"),
  "capture.saveCurrent": message("Save current view as…", "当前画面另存为…", "目前畫面另存為…", "現在の表示を別名で保存…", "Guardar vista actual como…", "Enregistrer la vue actuelle sous…", "Aktuelle Ansicht speichern unter…"),
  "capture.copyCurrent": message("Copy current view", "复制当前画面", "複製目前畫面", "現在の表示をコピー", "Copiar vista actual", "Copier la vue actuelle", "Aktuelle Ansicht kopieren"),
  "capture.savePreview": message("Save full preview as…", "完整预览图另存为…", "完整預覽圖另存為…", "完全プレビューを別名で保存…", "Guardar vista previa completa como…", "Enregistrer l’aperçu complet sous…", "Vollständige Vorschau speichern unter…"),
  "capture.copyPreview": message("Copy full preview", "复制完整预览图", "複製完整預覽圖", "完全プレビューをコピー", "Copiar vista previa completa", "Copier l’aperçu complet", "Vollständige Vorschau kopieren"),
  "capture.saveTitle": message("Save PNG image", "保存 PNG 图像", "儲存 PNG 影像", "PNG 画像を保存", "Guardar imagen PNG", "Enregistrer l’image PNG", "PNG-Bild speichern"),
  "capture.generatingCurrent": message("Capturing the current view…", "正在抓取当前画面…", "正在擷取目前畫面…", "現在の表示をキャプチャ中…", "Capturando la vista actual…", "Capture de la vue actuelle…", "Aktuelle Ansicht wird aufgenommen…"),
  "capture.generatingPreview": message("Rendering the full preview…", "正在生成完整预览图…", "正在產生完整預覽圖…", "完全プレビューを生成中…", "Generando la vista previa completa…", "Génération de l’aperçu complet…", "Vollständige Vorschau wird erzeugt…"),
  "capture.saved": message("PNG image saved", "PNG 图像已保存", "PNG 影像已儲存", "PNG 画像を保存しました", "Imagen PNG guardada", "Image PNG enregistrée", "PNG-Bild gespeichert"),
  "capture.copied": message("Image copied to the clipboard", "图像已复制到剪贴板", "影像已複製到剪貼簿", "画像をクリップボードにコピーしました", "Imagen copiada al portapapeles", "Image copiée dans le presse-papiers", "Bild in die Zwischenablage kopiert"),
  "capture.failed": message("Could not capture the image", "无法完成图像抓拍", "無法完成影像擷取", "画像をキャプチャできませんでした", "No se pudo capturar la imagen", "Impossible de capturer l’image", "Bild konnte nicht aufgenommen werden"),
  "capture.stale": message("The view changed; capture was canceled", "画面已变化，本次抓拍已取消", "畫面已變更，本次擷取已取消", "表示が変更されたため、キャプチャを中止しました", "La vista cambió; se canceló la captura", "La vue a changé ; la capture a été annulée", "Ansicht geändert; Aufnahme wurde abgebrochen"),
  ...
  "backend.captureInvalidPng": message("The captured image is not valid PNG data", "抓拍结果不是有效的 PNG 数据", "擷取結果不是有效的 PNG 資料", "キャプチャ結果は有効な PNG データではありません", "La captura no contiene datos PNG válidos", "La capture ne contient pas de données PNG valides", "Aufnahme enthält keine gültigen PNG-Daten"),
  "backend.captureOverwritesSource": message("The PNG path cannot overwrite the currently open RAW source", "PNG 路径不能覆盖当前打开的 RAW 源文件", "PNG 路徑不可覆寫目前開啟的 RAW 來源檔案", "PNG の保存先に現在開いている RAW ソースを上書きできません", "La ruta PNG no puede sobrescribir el RAW de origen abierto", "Le chemin PNG ne peut pas écraser le RAW source ouvert", "PNG-Pfad darf die geöffnete RAW-Quelldatei nicht überschreiben"),
  "backend.captureSaveFailed": message("Could not save the PNG image", "无法保存 PNG 图像", "無法儲存 PNG 影像", "PNG 画像を保存できませんでした", "No se pudo guardar la imagen PNG", "Impossible d’enregistrer l’image PNG", "PNG-Bild konnte nicht gespeichert werden"),
