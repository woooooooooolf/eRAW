...
const CATALOG = {
  ...
  "backend.displayRangeTaskFailed": message("Display normalization failed", "显示归一化失败", "顯示正規化失敗", "表示の正規化に失敗しました", "Error al normalizar la visualización", "Échec de la normalisation de l’affichage", "Anzeigenormalisierung fehlgeschlagen"),
  "backend.displayRangeInvalidFrame": message("The display-normalization frame is unavailable", "显示归一化帧不可用", "顯示正規化影格不可用", "表示正規化の対象フレームは利用できません", "El fotograma para normalizar la visualización no está disponible", "L’image de normalisation de l’affichage n’est pas disponible", "Der Frame für die Anzeigenormalisierung ist nicht verfügbar"),
  "backend.displayInvalidWindow": message("The display black and white points are invalid", "显示黑点与白点无效", "顯示黑點與白點無效", "表示の黒点と白点が無効です", "Los puntos negro y blanco de visualización no son válidos", "Les points noir et blanc de l’affichage ne sont pas valides", "Schwarz- und Weißpunkt der Anzeige sind ungültig"),
  ...
  "presentation.displayExposure": message("Display exposure", "显示曝光", "顯示曝光", "表示露出", "Exposición de visualización", "Exposition d’affichage", "Anzeigehelligkeit"),
  "presentation.displayExposureHint": message("Applies the same brightness gain to every display mode and PNG capture; DN, statistics, processing, and exports remain unchanged", "对所有显示模式及 PNG 抓拍应用同一亮度增益，不改变 DN、统计、处理或导出", "對所有顯示模式及 PNG 擷取套用相同亮度增益，不改變 DN、統計、處理或匯出", "すべての表示モードと PNG キャプチャーに同じ明るさゲインを適用し、DN、統計、処理、エクスポートは変更しません", "Aplica la misma ganancia de brillo a todos los modos y capturas PNG; no cambia DN, estadísticas, procesamiento ni exportaciones", "Applique le même gain de luminosité à tous les modes et captures PNG, sans changer les DN, statistiques, traitements ni exports", "Wendet dieselbe Helligkeitsverstärkung auf alle Anzeigemodi und PNG-Aufnahmen an; DN, Statistik, Verarbeitung und Export bleiben unverändert"),
  "presentation.decreaseDisplayExposure": message("Decrease display exposure", "减小显示曝光", "減小顯示曝光", "表示露出を下げる", "Reducir la exposición de visualización", "Réduire l’exposition d’affichage", "Anzeigehelligkeit verringern"),
  "presentation.increaseDisplayExposure": message("Increase display exposure", "增加显示曝光", "增加顯示曝光", "表示露出を上げる", "Aumentar la exposición de visualización", "Augmenter l’exposition d’affichage", "Anzeigehelligkeit erhöhen"),
  "presentation.displayWindow": message("Display black / white point", "显示黑点 / 白点", "顯示黑點 / 白點", "表示黒点 / 白点", "Punto negro / blanco de visualización", "Point noir / blanc d’affichage", "Anzeige-Schwarz- / Weißpunkt"),
  "presentation.displayWindowHint": message("Maps DN generated by every display mode between the same black and white points before 8-bit quantization", "在 8-bit 量化之前，将所有显示模式生成的 DN 按同一黑点与白点映射到完整预览范围", "在 8-bit 量化之前，將所有顯示模式產生的 DN 依相同黑點與白點映射到完整預覽範圍", "8-bit 量子化の前に、すべての表示モードが生成した DN を同じ黒点と白点でプレビュー全範囲へ割り当てます", "Asigna los DN generados por todos los modos entre los mismos puntos negro y blanco antes de cuantizar a 8 bits", "Mappe les DN produits par tous les modes entre les mêmes points noir et blanc avant la quantification 8 bits", "Bildet die von allen Anzeigemodi erzeugten DN vor der 8-Bit-Quantisierung mit denselben Schwarz- und Weißpunkten ab"),
  "presentation.displayBlackPoint": message("Display black point", "显示黑点", "顯示黑點", "表示黒点", "Punto negro de visualización", "Point noir de l’affichage", "Schwarzpunkt der Anzeige"),
  "presentation.displayWhitePoint": message("Display white point", "显示白点", "顯示白點", "表示白点", "Punto blanco de visualización", "Point blanc de l’affichage", "Weißpunkt der Anzeige"),
  "presentation.displayAuto": message("Display auto normalization", "显示自动归一化", "顯示自動正規化", "表示の自動正規化", "Normalización automática de visualización", "Normalisation automatique de l’affichage", "Automatische Anzeigenormalisierung"),
  "presentation.displayAutoHint": message("Scans every L0 original DN in the current full frame, ignores missing pixels, and uses the All-channel P1/P99 as the shared black and white points for every display mode", "精确扫描当前整帧的全部 L0 原始 DN，忽略缺失像素，并将 All 组 P1/P99 作为所有显示模式共用的黑点与白点", "精確掃描目前整個影格的全部 L0 原始 DN，忽略缺失像素，並將 All 組 P1/P99 作為所有顯示模式共用的黑點與白點", "現在のフレーム全体の L0 元 DN を走査し、欠損画素を除外して All チャンネルの P1/P99 を全表示モード共通の黒点と白点に設定します", "Examina todos los DN originales L0 del fotograma, ignora píxeles ausentes y usa P1/P99 de All como puntos compartidos para todos los modos", "Analyse tous les DN L0 d’origine du frame, ignore les pixels manquants et emploie P1/P99 de All comme points communs à tous les modes", "Durchsucht alle ursprünglichen L0-DN des Frames, ignoriert fehlende Pixel und nutzt All-P1/P99 als gemeinsame Schwarz- und Weißpunkte aller Anzeigemodi"),
  ...
  "presentation.resetDisplay": message("Reset", "重置", "重設", "リセット", "Restablecer", "Réinitialiser", "Zurücksetzen"),
  "presentation.displayAutoApplied": message("Display range set to {blackPoint}–{whitePoint} DN", "显示范围已设为 {blackPoint}–{whitePoint} DN", "顯示範圍已設為 {blackPoint}–{whitePoint} DN", "表示範囲を {blackPoint}–{whitePoint} DN に設定しました", "Rango de visualización establecido en {blackPoint}–{whitePoint} DN", "Plage d’affichage réglée sur {blackPoint}–{whitePoint} DN", "Anzeigebereich auf {blackPoint}–{whitePoint} DN gesetzt"),
  "presentation.displayAutoUnavailable": message("This frame has no usable DN range; the current display range was kept", "当前帧没有可用的 DN 范围，已保留现有显示范围", "目前影格沒有可用的 DN 範圍，已保留現有顯示範圍", "このフレームには使用可能な DN 範囲がないため、現在の表示範囲を維持しました", "Este fotograma no tiene un rango DN utilizable; se conservó el rango de visualización actual", "Cette image ne présente aucune plage DN exploitable ; la plage d’affichage actuelle a été conservée", "Dieser Frame hat keinen nutzbaren DN-Bereich; der aktuelle Anzeigebereich wurde beibehalten"),
