...
export const HELP_SECTIONS: readonly HelpSection[] = [
  ...
  },
  {
    ...
    body: `
      ...
      <div class="help-card-grid">
        ...
        <article><h3>像素联动</h3><p>主画布达到 12× 后，鼠标像素或 <kbd>P</kbd> 输入的坐标会以行列柔光显示，并在 Row 与 Column Profile 同时标出精确点。手动定位会保留到下一次画布或 Profile 鼠标交互，随后由鼠标接管，离开后不会恢复旧定位点；ROI 外没有统计标记。</p></article>
  ...
  },
  {
    ...
    body: `
      ...
      ${admonition("supplement", "手册版本", "本中文技术参考与 eRAW V0.6.0 当前实现同步。后续增加 Packing、处理算法或统计口径时，应把手册公式、测试和程序版本作为同一项变更维护。")}`,
  },
];
