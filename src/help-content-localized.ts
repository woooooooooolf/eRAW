  ...
  sections: {
    ...
    charts: {
      ...
      blocks: [
        ...
        { title: "Pixel/Profile linking", text: "At 12× or above, the canvas pointer or a coordinate entered with <code>P</code> softly highlights both source row and column and marks their exact points in both Profiles. Canvas hover temporarily overrides the last manual location and restores it on leave. Hovering a Row or Column Profile highlights that physical band in the canvas. Linking never pans or zooms either view, stays invisible when outside the current range or statistics ROI, is disabled below 12×, and does not apply to Histogram." },
      ],
  ...
  sections: {
    ...
    charts: {
      ...
      blocks: [
        ...
        { title: "像素與 Profile 聯動", text: "畫布達到 12× 後，滑鼠像素或按 <code>P</code> 輸入的座標會柔光標示來源行列，並在兩個 Profile 標出精確點。畫布滑鼠暫時接管手動定位，離開後恢復；懸停 Row/Column Profile 則反向標示實體行或列。聯動不平移或縮放，超出目前範圍或統計 ROI 時保持不可見，低於 12× 關閉，Histogram 不參與。" },
  ...
  sections: {
    ...
    charts: { kicker: "有界な可視化", title: "統計 chart の集約と読み方", summary: "正確な summary を保ち、interactive renderer に渡す点数だけを削減します。", level: "上級", readingTime: "約 8 分",
      blocks: [
        ...
        { title: "Pixel/Profile 連動", text: "Canvas が 12× 以上では、pointer または <code>P</code> で入力した座標の source row/column を soft glow で示し、両 Profile の exact point を表示します。Canvas hover は手動位置を一時的に優先し、leave 後に復元します。Row/Column Profile hover は対応する物理行/列を逆方向に強調します。どの view も pan/zoom せず、現在範囲または統計 ROI 外では見えず、12× 未満と Histogram では無効です。" },
