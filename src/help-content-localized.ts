...
const EN_COPY: LocaleCopy = {
  ...
  sections: {
    ...
    rendering: {
      ...
      blocks: [
        { title: "Display range and exposure", text: "RAW intensity and CFA mosaic use explicit black and white points <code>0≤B&lt;W≤2^bitDepth−1</code> before 8-bit quantization, then apply <code>2^EV</code> during final presentation. Other modes use the full DN range. Neither adjustment changes inspected DN, statistics, processing, or export." },
        { title: "One-shot auto range", text: "P1–P99 scans every L0 All DN in the current full frame, excludes missing samples, and sets the RAW/CFA black and white points once. It falls back to Min/Max when P1 equals P99; a flat or fully missing frame keeps the current range. ROI and the full statistics result are not used." },
...
const ZH_TW_COPY: LocaleCopy = {
  ...
  sections: {
    ...
    rendering: {
      ...
      blocks: [
        { title: "顯示範圍與曝光", text: "RAW 強度與 CFA 點陣在 8-bit 量化前使用明確的黑點、白點 <code>0≤B&lt;W≤2^bitDepth−1</code>，最終呈現再套用 <code>2^EV</code>；其它模式固定使用全量程。這不改變 DN、統計、處理或匯出。" },
        { title: "一次性自動範圍", text: "P1–P99 精確掃描目前整幀 L0 All DN、排除缺失樣本並設定 RAW/CFA 黑白點。P1 等於 P99 時回退 Min/Max；平坦或全缺失影格保留目前範圍。它不使用 ROI 或完整統計結果。" },
...
const JA_COPY: LocaleCopy = {
  ...
  sections: {
    ...
    rendering: { kicker: "DN から画面へ", title: "表示正規化、tile、LOD", summary: "raw 数値、8-bit preview、tile rendering、detail level を分離して理解します。", level: "上級", readingTime: "約 10 分",
      blocks: [
        { title: "表示範囲と露出", text: "RAW 強度と CFA モザイクは 8-bit 量子化前に黒点・白点 <code>0≤B&lt;W≤2^bitDepth−1</code> を使い、最終表示で <code>2^EV</code> を適用します。他モードは全範囲固定です。DN、統計、処理、export は変更しません。" },
        { title: "一回限りの自動範囲", text: "P1–P99 は現在のフレーム全体の L0 All DN を正確に走査し、missing を除外して RAW/CFA の黒点と白点を設定します。P1=P99 なら Min/Max にフォールバックし、平坦または全欠損なら現在範囲を維持します。ROI と完全な統計結果は使いません。" },
...
const ES_COPY: LocaleCopy = {
  ...
  sections: {
    ...
    rendering: { kicker: "DE DN A PANTALLA", title: "Normalización, tiles y LOD", summary: "Separe el valor RAW de la vista previa de 8 bits, el render por tiles y la selección LOD.", level: "Avanzado", readingTime: "Unos 10 min",
      blocks: [{ title: "Rango y exposición", text: "La intensidad RAW y el mosaico CFA usan puntos negro y blanco explícitos <code>0≤B&lt;W≤2^bitDepth−1</code> antes de cuantizar a 8 bits y aplican <code>2^EV</code> en la presentación final. Los demás modos usan el rango completo. No cambia DN, estadísticas, procesamiento ni exportación." }, { title: "Rango automático puntual", text: "P1–P99 escanea exactamente todos los DN L0 All del fotograma completo actual, excluye missing y fija los puntos RAW/CFA. Si P1=P99 usa Min/Max; un fotograma plano o totalmente ausente conserva el rango. No usa ROI ni el resultado estadístico completo." }, { title: "Tiles y LOD", text: "Cada render lleva generation/revision y un resultado antiguo se rechaza como stale. Los LOD RAW/CFA conservan sitios; la vista completa elige el LOD de mayor resolución con lado largo ≤4096 px." }],
...
const FR_COPY: LocaleCopy = {
  ...
  sections: {
    ...
    rendering: { kicker: "DU DN À L'ÉCRAN", title: "Normalisation, tiles et LOD", summary: "Séparez la valeur RAW de la preview 8-bit, du rendu par tiles et du choix LOD.", level: "Avancé", readingTime: "Environ 10 min", blocks: [{ title: "Plage et exposition", text: "L’intensité RAW et la mosaïque CFA utilisent des points noir et blanc explicites <code>0≤B&lt;W≤2^bitDepth−1</code> avant la quantification 8 bits, puis <code>2^EV</code> à la présentation finale. Les autres modes emploient la pleine plage. Ni DN, statistiques, traitement, ni export ne changent." }, { title: "Plage automatique ponctuelle", text: "P1–P99 analyse exactement tous les DN L0 All du frame entier actuel, exclut les missing et règle les points RAW/CFA. Si P1=P99, Min/Max sert de repli ; un frame plat ou entièrement manquant conserve la plage. Ni ROI ni résultat statistique complet ne sont utilisés." }, { title: "Tiles et LOD", text: "Chaque rendu porte generation/revision ; un résultat ancien est rejeté comme stale. Les LOD RAW/CFA gardent les sites et la preview complète choisit le meilleur niveau dont le grand côté ≤4096 px." }], formulaHeading: "Équations d'affichage et LOD", formulaLabels: ["Plage valide H > L", "Exposition d’affichage", "LOD idéal"], note: { kind: "supplement", title: "La preview complète reste une preview", text: "Elle partage la présentation, mais ne remplace pas l'export DN L0 ni les statistiques L0 exactes." } },
...
const DE_COPY: LocaleCopy = {
  ...
  sections: {
    ...
    rendering: { kicker: "VON DN ZUM BILDSCHIRM", title: "Anzeigenormalisierung, Tiles und LOD", summary: "Trennen Sie RAW-Wert, 8-bit Preview, Tile-Rendering und LOD-Auswahl.", level: "Fortgeschritten", readingTime: "Ca. 10 Min.", blocks: [{ title: "Bereich und Belichtung", text: "RAW-Intensität und CFA-Mosaik verwenden vor der 8-Bit-Quantisierung explizite Schwarz-/Weißpunkte <code>0≤B&lt;W≤2^bitDepth−1</code> und bei der finalen Darstellung <code>2^EV</code>. Andere Modi nutzen den Vollbereich. DN, Statistik, Verarbeitung und Export bleiben unverändert." }, { title: "Einmaliger Auto-Bereich", text: "P1–P99 scannt exakt alle L0-All-DN des aktuellen Vollframes, schließt missing aus und setzt die RAW/CFA-Punkte. Bei P1=P99 wird Min/Max genutzt; ein flacher oder völlig fehlender Frame behält den Bereich. ROI und vollständiges Statistikergebnis werden nicht verwendet." }, { title: "Tiles und LOD", text: "Jedes Rendering trägt generation/revision; alte Resultate werden als stale verworfen. RAW/CFA-LOD bewahren Sites; Full Preview wählt die höchste Auflösung mit langer Kante ≤4096 px." }], formulaHeading: "Anzeige- und LOD-Gleichungen", formulaLabels: ["Gültiger Bereich H > L", "Anzeigebelichtung", "Idealer LOD"], note: { kind: "supplement", title: "Full Preview bleibt Preview", text: "Sie teilt die aktuelle Darstellung, ersetzt aber weder L0-DN-Export noch exakte L0-Statistik." } },
