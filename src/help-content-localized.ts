...
const FORMULAS: Partial<Record<SectionId, readonly (readonly string[])[]>> = {
  ...
  rendering: [
    ...
    [String.raw`\mathrm{preview}_{EV}=\operatorname{clamp}(\mathrm{preview}\cdot2^{EV},0,255)`],
  ...
  sections: {
    ...
    workflow: {
      ...
      blocks: [
        ...
        { title: "View and ROI", text: "The wheel zooms around the image point under the pointer and left drag pans. <code>Ctrl+0</code> fits the image; <code>Ctrl+1</code> returns to 100%. In multi-frame files, <code>[</code> and <code>]</code> move to the previous and next frame without wrapping. Frame changes of the same dimensions retain the view, while a dimension change clears coordinate-dependent ROI state." },
        { title: "Task shortcuts", text: "Use <code>M</code> or <code>Shift+M</code> to cycle forward or backward through the primary display modes supported by the current CFA; R/G/B channel views are excluded. Use <code>R</code> or <code>Shift+R</code> for mouse or coordinate ROI, <code>P</code> to locate a pixel, <code>Ctrl+I</code> for statistics, <code>Ctrl+E</code> for deterministic export, <code>F11</code> for native fullscreen, and <code>F1</code> for this manual. Except for <code>F1</code> and <code>Esc</code>, main-window shortcuts pause while an input, pop-up menu, or modal dialog is active, and modifier combinations must match exactly." },
      ],
    ...
    rendering: {
      ...
      blocks: [
        { title: "Display range and exposure", text: "Every display mode uses the same black and white points <code>0≤B&lt;W≤2^bitDepth−1</code> after producing its DN and before 8-bit quantization, then applies the same <code>2^EV</code> during final presentation. Neither adjustment changes inspected DN, statistics, processing, or export." },
        { title: "One-shot auto range", text: "P1–P99 scans every original L0 All DN in the current full frame, excludes missing samples, and sets the shared black and white points for every display mode. It falls back to Min/Max when P1 equals P99; a flat or fully missing frame keeps the current range. ROI, processed DN, and the full statistics result are not used." },
      ...
      formulaHeading: "Display and LOD equations", formulaLabels: ["Valid range H > L", "Display exposure", "Ideal LOD"],
    ...
    charts: {
      ...
      blocks: [
        ...
        { title: "Pixel/Profile linking", text: "At 12× or above, the canvas pointer or a coordinate entered with <code>P</code> softly highlights both source row and column and marks their exact points in both Profiles. A manual location remains until the next canvas or Profile pointer interaction takes over, and the old location is not restored on leave. Hovering a Row or Column Profile highlights that physical band in the canvas. Linking never pans or zooms either view, stays invisible when outside the current range or statistics ROI, is disabled below 12×, and does not apply to Histogram." },
      ],
    ...
    glossary: {
      ...
      note: { kind: "supplement", title: "Manual version", text: "This technical reference follows eRAW V0.6.0. Packing, processing, or statistical changes should update code, equations, tests, and all language catalogs as one versioned change." },
    },
  },
};
  ...
  sections: {
    ...
    workflow: {
      ...
      blocks: [
        ...
        { title: "視圖與 ROI", text: "滾輪以指標下的影像點為錨縮放，左鍵拖曳平移；<code>Ctrl+0</code> 適合視窗，<code>Ctrl+1</code> 回到 100%。多影格檔案以 <code>[</code>/<code>]</code> 切換上一或下一影格且不循環；同尺寸換幀保留視圖，尺寸改變會清除依賴座標的 ROI。" },
        { title: "常用快捷鍵", text: "<code>M</code>/<code>Shift+M</code> 向前或向後循環目前 CFA 支援的主要顯示模式，不包含 R/G/B 通道；<code>R</code>/<code>Shift+R</code> 選取 ROI，<code>P</code> 定位像素，<code>Ctrl+I</code> 統計，<code>Ctrl+E</code> 匯出，<code>F11</code> 原生全螢幕，<code>F1</code> 開啟本手冊。除 <code>F1</code> 和 <code>Esc</code> 外，主視窗快捷鍵在輸入控件、彈出選單或模態對話框啟用時暫停，組合鍵的修飾鍵必須完全相符。" },
    ...
    rendering: {
      ...
      blocks: [
        { title: "顯示範圍與曝光", text: "所有顯示模式在產生各自 DN 後、8-bit 量化前共用黑點與白點 <code>0≤B&lt;W≤2^bitDepth−1</code>，最終呈現再套用同一 <code>2^EV</code>。這不改變 DN、統計、處理或匯出。" },
        { title: "一次性自動範圍", text: "P1–P99 精確掃描目前整幀 L0 原始 All DN、排除缺失樣本並設定所有顯示模式共用的黑白點。P1 等於 P99 時回退 Min/Max；平坦或全缺失影格保留目前範圍。它不使用 ROI、處理後 DN 或完整統計結果。" },
      ...
      ], formulaHeading: "顯示與 LOD 公式", formulaLabels: ["有效範圍 H > L", "顯示曝光", "理想 LOD"], note: { kind: "supplement", title: "完整預覽仍是預覽", text: "它保留目前顯示語義，適合分享，但不能取代 L0 DN 匯出或精確 L0 統計。" },
    },
    inspection: {
    ...
    charts: {
      ...
      blocks: [
        ...
        { title: "像素與 Profile 聯動", text: "畫布達到 12× 後，滑鼠像素或按 <code>P</code> 輸入的座標會柔光標示來源行列，並在兩個 Profile 標出精確點。手動定位保留到下一次畫布或 Profile 滑鼠互動，隨後由滑鼠接管，離開後不會恢復舊定位點；懸停 Row/Column Profile 則反向標示實體行或列。聯動不平移或縮放，超出目前範圍或統計 ROI 時保持不可見，低於 12× 關閉，Histogram 不參與。" },
    ...
    glossary: {
      ...
      ], note: { kind: "supplement", title: "手冊版本", text: "本技術參考與 eRAW V0.6.0 同步。Packing、處理或統計變更應把程式、公式、測試與所有語言目錄作為同一版本變更維護。" },
    },
  },
};
  ...
  sections: {
    ...
    workflow: { kicker: "操作モデル", title: "ファイルセッション、キャンバス、ショートカット", summary: "現在の document、frame、表示状態の関係を理解し、古い結果を新しいファイルへ持ち込まないようにします。", level: "入門", readingTime: "約 6 分",
      blocks: [
        ...
        { title: "表示と操作", text: "wheel は pointer 下を基準に zoom、左 drag は pan です。<code>Ctrl+0</code> は fit、<code>Ctrl+1</code> は 100%。複数フレームでは <code>[</code>/<code>]</code> で前後へ移動し、端では循環しません。<code>M</code>/<code>Shift+M</code> は現在の CFA が対応する主要表示モードだけを順方向/逆方向に切り替え、R/G/B 表示は対象外です。<code>R</code> は ROI、<code>P</code> は pixel、<code>Ctrl+I</code> は統計、<code>Ctrl+E</code> は export、<code>F1</code> は本マニュアルです。<code>F1</code> と <code>Esc</code> を除くメインウィンドウのショートカットは、入力欄、ポップアップメニュー、モーダルダイアログの操作中は停止し、修飾キーも完全一致で判定されます。" },
    ...
    rendering: { kicker: "DN から画面へ", title: "表示正規化、tile、LOD", summary: "raw 数値、8-bit preview、tile rendering、detail level を分離して理解します。", level: "上級", readingTime: "約 10 分",
      blocks: [
        { title: "表示範囲と露出", text: "すべての表示モードは各 DN を生成した後、8-bit 量子化前に共通の黒点・白点 <code>0≤B&lt;W≤2^bitDepth−1</code> を使い、最終表示で同じ <code>2^EV</code> を適用します。DN、統計、処理、export は変更しません。" },
        { title: "一回限りの自動範囲", text: "P1–P99 は現在のフレーム全体の L0 元 All DN を正確に走査し、missing を除外して全表示モード共通の黒点と白点を設定します。P1=P99 なら Min/Max にフォールバックし、平坦または全欠損なら現在範囲を維持します。ROI、処理後 DN、完全な統計結果は使いません。" },
      ...
      ], formulaHeading: "表示と LOD の式", formulaLabels: ["有効範囲 H > L", "表示露出", "理想 LOD"], note: { kind: "supplement", title: "full preview も preview", text: "現在の表示 semantics を共有する画像であり、L0 DN export や正確な L0 統計の代替ではありません。" } },
    ...
    charts: { kicker: "有界な可視化", title: "統計 chart の集約と読み方", summary: "正確な summary を保ち、interactive renderer に渡す点数だけを削減します。", level: "上級", readingTime: "約 8 分",
      blocks: [
        ...
        { title: "Pixel/Profile 連動", text: "Canvas が 12× 以上では、pointer または <code>P</code> で入力した座標の source row/column を soft glow で示し、両 Profile の exact point を表示します。手動位置は次の Canvas または Profile pointer 操作まで保持され、その後は pointer が引き継ぎ、leave 後に古い位置を復元しません。Row/Column Profile hover は対応する物理行/列を逆方向に強調します。どの view も pan/zoom せず、現在範囲または統計 ROI 外では見えず、12× 未満と Histogram では無効です。" },
    ...
    glossary: { kicker: "用語と主張", title: "用語集と結果の解釈", summary: "RAW workflow の用語を統一し、eRAW の結果が何を意味するかを明確にします。", level: "入門", readingTime: "約 7 分",
      ...
      ], note: { kind: "supplement", title: "マニュアル版", text: "本リファレンスは eRAW V0.6.0 に対応します。Packing、処理、統計の変更時は code、式、test、全言語 catalog を同時更新します。" } },
  },
};
...
const ES_COPY: LocaleCopy = {
  ...
  sections: {
    ...
    workflow: { kicker: "MODELO OPERATIVO", title: "Sesiones de archivo, lienzo y atajos", summary: "Relacione documento, frame y estado de vista para no reutilizar resultados obsoletos.", level: "Introducción", readingTime: "Unos 6 min",
      blocks: [{ title: "Ciclo del documento", text: "Solo hay un documento RAW activo. Abrir otro archivo o aplicar un descriptor invalida tiles y estadísticas anteriores. <code>Ctrl+W</code> libera el mapeo de solo lectura sin borrar preferencias guardadas." }, { title: "Vista y tareas", text: "La rueda amplía bajo el puntero y el arrastre izquierdo desplaza. <code>Ctrl+0</code> ajusta y <code>Ctrl+1</code> vuelve al 100%. En archivos con varios fotogramas, <code>[</code>/<code>]</code> retrocede o avanza sin volver al otro extremo. <code>M</code>/<code>Shift+M</code> recorre hacia delante o atrás solo los modos de visualización principales compatibles con el CFA actual; R/G/B quedan fuera. <code>R</code> crea ROI, <code>Ctrl+I</code> abre estadísticas, <code>Ctrl+E</code> exporta y <code>F1</code> abre este manual. Salvo <code>F1</code> y <code>Esc</code>, los atajos de la ventana principal se suspenden mientras haya un campo, menú emergente o diálogo modal activo, y los modificadores deben coincidir exactamente." }],
    ...
    rendering: { kicker: "DE DN A PANTALLA", title: "Normalización, tiles y LOD", summary: "Separe el valor RAW de la vista previa de 8 bits, el render por tiles y la selección LOD.", level: "Avanzado", readingTime: "Unos 10 min",
      blocks: [{ title: "Rango y exposición", text: "Todos los modos usan los mismos puntos negro y blanco <code>0≤B&lt;W≤2^bitDepth−1</code> después de producir sus DN y antes de cuantizar a 8 bits, y aplican el mismo <code>2^EV</code> en la presentación final. No cambia DN, estadísticas, procesamiento ni exportación." }, { title: "Rango automático puntual", text: "P1–P99 escanea exactamente todos los DN originales L0 All del fotograma completo, excluye missing y fija los puntos compartidos por todos los modos. Si P1=P99 usa Min/Max; un fotograma plano o totalmente ausente conserva el rango. No usa ROI, DN procesados ni el resultado estadístico completo." }, { title: "Tiles y LOD", text: "Cada render lleva generation/revision y un resultado antiguo se rechaza como stale. Los LOD RAW/CFA conservan sitios; la vista completa elige el LOD de mayor resolución con lado largo ≤4096 px." }],
      formulaHeading: "Ecuaciones de pantalla y LOD", formulaLabels: ["Rango válido H > L", "Exposición de pantalla", "LOD ideal"], note: { kind: "supplement", title: "La vista completa sigue siendo preview", text: "Sirve para compartir la presentación, pero no sustituye DN L0 ni estadísticas exactas L0." } },
    ...
    charts: { kicker: "VISUALIZACIÓN ACOTADA", title: "Agregación y lectura de gráficos", summary: "Conserve resúmenes exactos reduciendo solo los puntos enviados al render interactivo.", level: "Avanzado", readingTime: "Unos 8 min",
      blocks: [{ title: "Histogram", text: "Si hay más de 4096 bins, se suman bins consecutivos y se dibuja su punto medio. Recuentos, media, varianza y percentiles siguen exactos." }, { title: "Profile", text: "Los Profiles largos conservan primero, último y mínimos/máximos ordenados por bucket para mantener picos con un máximo de 4096 puntos." }, { title: "Enlace píxel/Profile", text: "A 12× o más, el puntero del lienzo o una coordenada introducida con <code>P</code> resalta suavemente fila y columna y marca puntos exactos en ambos Profiles. La ubicación manual se mantiene hasta la siguiente interacción del puntero en el lienzo o un Profile; después el puntero toma el control y la ubicación anterior no se restaura al salir. Al pasar por Row/Column Profile se resalta la banda física correspondiente. El enlace no desplaza ni amplía ninguna vista, queda invisible fuera del rango o ROI estadístico, se desactiva por debajo de 12× y no se aplica a Histogram." }],
    ...
    glossary: { kicker: "TÉRMINOS Y CONCLUSIONES", title: "Glosario e interpretación", summary: "Unifique términos RAW y determine qué conclusiones admite eRAW.", level: "Introducción", readingTime: "Unos 7 min",
      ...
      note: { kind: "supplement", title: "Versión del manual", text: "Esta referencia corresponde a eRAW V0.6.0. Cambios de Packing, procesamiento o estadísticas deben actualizar código, fórmulas, tests y todos los idiomas juntos." } },
  },
};
...
const FR_COPY: LocaleCopy = {
  ...
  sections: {
    ...
    workflow: { kicker: "MODÈLE D'UTILISATION", title: "Sessions de fichier, canevas et raccourcis", summary: "Reliez document, frame et vue pour éviter de réutiliser des résultats périmés.", level: "Débutant", readingTime: "Environ 6 min", blocks: [{ title: "Durée de vie", text: "Un seul document RAW est actif. Ouvrir un autre fichier ou appliquer un descripteur invalide les anciens tiles et statistiques. <code>Ctrl+W</code> libère le mapping en lecture seule sans effacer les préférences." }, { title: "Vue et tâches", text: "La molette zoome sous le pointeur et le glisser gauche déplace. <code>Ctrl+0</code> ajuste et <code>Ctrl+1</code> affiche 100 %. Dans un fichier multiframe, <code>[</code>/<code>]</code> passe à l’image précédente ou suivante sans boucler. <code>M</code>/<code>Shift+M</code> parcourt dans les deux sens les seuls modes d’affichage principaux acceptés par le CFA actuel ; R/G/B sont exclus. <code>R</code> crée un ROI, <code>Ctrl+I</code> ouvre les statistiques, <code>Ctrl+E</code> exporte et <code>F1</code> ouvre ce manuel. Sauf <code>F1</code> et <code>Esc</code>, les raccourcis de la fenêtre principale sont suspendus pendant la saisie, l’ouverture d’un menu contextuel ou d’une boîte de dialogue modale, et les modificateurs doivent correspondre exactement." }], note: { kind: "supplement", title: "La présentation ne modifie pas les données", text: "Thème, langue, police, panneau, vitesse, cache et animations n'altèrent ni octets, DN, CFA, statistiques, ni export déterministe." } },
    ...
    rendering: { kicker: "DU DN À L'ÉCRAN", title: "Normalisation, tiles et LOD", summary: "Séparez la valeur RAW de la preview 8-bit, du rendu par tiles et du choix LOD.", level: "Avancé", readingTime: "Environ 10 min", blocks: [{ title: "Plage et exposition", text: "Tous les modes emploient les mêmes points noir et blanc <code>0≤B&lt;W≤2^bitDepth−1</code> après production de leurs DN et avant la quantification 8 bits, puis le même <code>2^EV</code> à la présentation finale. Ni DN, statistiques, traitement, ni export ne changent." }, { title: "Plage automatique ponctuelle", text: "P1–P99 analyse exactement tous les DN L0 All d’origine du frame entier, exclut les missing et règle les points communs à tous les modes. Si P1=P99, Min/Max sert de repli ; un frame plat ou entièrement manquant conserve la plage. Ni ROI, DN traités, ni résultat statistique complet ne sont utilisés." }, { title: "Tiles et LOD", text: "Chaque rendu porte generation/revision ; un résultat ancien est rejeté comme stale. Les LOD RAW/CFA gardent les sites et la preview complète choisit le meilleur niveau dont le grand côté ≤4096 px." }], formulaHeading: "Équations d'affichage et LOD", formulaLabels: ["Plage valide H > L", "Exposition d’affichage", "LOD idéal"], note: { kind: "supplement", title: "La preview complète reste une preview", text: "Elle partage la présentation, mais ne remplace pas l'export DN L0 ni les statistiques L0 exactes." } },
    ...
    charts: { kicker: "VISUALISATION BORNÉE", title: "Agrégation et lecture des graphiques", summary: "Gardez les résumés exacts en réduisant seulement les points envoyés au rendu interactif.", level: "Avancé", readingTime: "Environ 8 min", blocks: [{ title: "Histogram", text: "Au-delà de 4096 bins, des bins consécutifs sont sommés et tracés à leur milieu. Comptes, moyenne, variance et percentiles restent exacts." }, { title: "Profile", text: "Les Profiles longs gardent premier, dernier et min/max ordonnés par bucket, préservant les pics avec au plus 4096 points." }, { title: "Lien pixel/Profile", text: "À partir de 12×, le pointeur du canvas ou une coordonnée saisie avec <code>P</code> met doucement en évidence ligne et colonne et marque les points exacts des deux Profiles. La position manuelle reste active jusqu'à la prochaine interaction du pointeur dans le canvas ou un Profile ; le pointeur prend alors le relais et l'ancienne position n'est pas restaurée en sortant. Survoler Row/Column Profile souligne la bande physique correspondante. Le lien ne déplace ni ne zoome aucune vue, reste invisible hors plage ou ROI statistique, est désactivé sous 12× et ne concerne pas Histogram." }], formulaHeading: "Équations de réduction", formulaLabels: ["Buckets Histogram", "Enveloppe min–max Profile"], note: { kind: "supplement", title: "La réduction ne change pas les résumés", text: "Seul le payload graphique diminue ; les statistiques exactes sont calculées avant." } },
...
const DE_COPY: LocaleCopy = {
  ...
  sections: {
    ...
    rendering: { kicker: "VON DN ZUM BILDSCHIRM", title: "Anzeigenormalisierung, Tiles und LOD", summary: "Trennen Sie RAW-Wert, 8-bit Preview, Tile-Rendering und LOD-Auswahl.", level: "Fortgeschritten", readingTime: "Ca. 10 Min.", blocks: [{ title: "Bereich und Belichtung", text: "Alle Anzeigemodi verwenden nach Erzeugung ihrer DN und vor der 8-Bit-Quantisierung dieselben Schwarz-/Weißpunkte <code>0≤B&lt;W≤2^bitDepth−1</code> und bei der finalen Darstellung dasselbe <code>2^EV</code>. DN, Statistik, Verarbeitung und Export bleiben unverändert." }, { title: "Einmaliger Auto-Bereich", text: "P1–P99 scannt exakt alle ursprünglichen L0-All-DN des aktuellen Vollframes, schließt missing aus und setzt die gemeinsamen Punkte aller Modi. Bei P1=P99 wird Min/Max genutzt; ein flacher oder völlig fehlender Frame behält den Bereich. ROI, verarbeitete DN und vollständiges Statistikergebnis werden nicht verwendet." }, { title: "Tiles und LOD", text: "Jedes Rendering trägt generation/revision; alte Resultate werden als stale verworfen. RAW/CFA-LOD bewahren Sites; Full Preview wählt die höchste Auflösung mit langer Kante ≤4096 px." }], formulaHeading: "Anzeige- und LOD-Gleichungen", formulaLabels: ["Gültiger Bereich H > L", "Anzeigebelichtung", "Idealer LOD"], note: { kind: "supplement", title: "Full Preview bleibt Preview", text: "Sie teilt die aktuelle Darstellung, ersetzt aber weder L0-DN-Export noch exakte L0-Statistik." } },
    ...
    charts: { kicker: "BEGRENZTE VISUALISIERUNG", title: "Aggregation und Lesen der Statistikdiagramme", summary: "Bewahren Sie exakte Kennwerte und reduzieren Sie nur Punkte für den interaktiven Renderer.", level: "Fortgeschritten", readingTime: "Ca. 8 Min.", blocks: [{ title: "Histogram", text: "Über 4096 Bins werden aufeinanderfolgende Bins summiert und am Mittelpunkt gezeichnet. Counts, Mittelwert, Varianz und Perzentile bleiben exakt." }, { title: "Profile", text: "Lange Profiles behalten First/Last und geordnete Min/Max je Bucket, sodass Peaks mit höchstens 4096 Punkten sichtbar bleiben." }, { title: "Pixel/Profile-Kopplung", text: "Ab 12× markiert der Canvas-Zeiger oder eine mit <code>P</code> eingegebene Koordinate Zeile und Spalte mit sanftem Leuchten und zeigt exakte Punkte in beiden Profiles. Die manuelle Position bleibt bis zur nächsten Zeigerinteraktion im Canvas oder einem Profile bestehen; danach übernimmt der Zeiger, und die alte Position wird beim Verlassen nicht wiederhergestellt. Hover über Row/Column Profile hebt das passende physische Band hervor. Die Kopplung verschiebt oder zoomt keine Ansicht, bleibt außerhalb von Bereich oder Statistik-ROI unsichtbar, ist unter 12× deaktiviert und gilt nicht für Histogram." }], formulaHeading: "Reduktionsgleichungen", formulaLabels: ["Histogram-Buckets", "Profile-Min–Max-Hülle"], note: { kind: "supplement", title: "Reduktion ändert keine Kennwerte", text: "Nur die Diagrammlast sinkt; exakte Statistik wird vorher berechnet." } },
